        ...
        <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
            ...
            <div className="flex-grow max-w-6xl mx-auto px-6 py-16 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {fees.map((fee, index) => (
                        <div key={index} className={`p-8 rounded-3xl border transition-all hover:-translate-y-2 hover:shadow-2xl ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100 shadow-xl'}`}>
                            <div className="text-center mb-6">
                                <span className={`inline-block px-3 py-1 rounded-xl text-xs font-bold uppercase mb-4 ${index === 0 ? 'bg-green-100 text-green-700' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'}`}>
                                    {fee.level}
                                </span>
