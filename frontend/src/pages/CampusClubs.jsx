      ...
      <div className="relative py-20 px-6 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight">
            Find Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-600">Community</span>
          </h1>
          <p className="text-xl opacity-80 mb-10 max-w-2xl mx-auto">
            Connect with like-minded students, learn new skills, and make lasting memories in one of our many campus clubs.
          </p>
          
          {/* Search Bar */}
          <div className={`flex items-center max-w-lg mx-auto p-2 rounded-xl shadow-2xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} transform hover:scale-[1.02] transition-all`}>
            <FiSearch className="ml-4 text-xl text-gray-400" />
            <input 
              type="text" 
              placeholder="Search for clubs (e.g. AWS, Chess)..." 
              className="flex-grow bg-transparent border-none focus:ring-0 px-4 py-3 text-lg outline-none text-gray-900 dark:text-white"
              value={searchTerm}
        ...
        {filteredClubs.length > 0 ? (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
             {filteredClubs.map(club => (
                <div key={club.id} className={`group relative rounded-3xl overflow-hidden border transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${
                  theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200 shadow-xl'
                }`}>
                  {/* Image */}
