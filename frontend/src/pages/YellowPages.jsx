...
const YellowPages = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'} transition-colors duration-300`}>
      ...
      {/* Success Toast */}
      {showSuccessToast && (
        <div className="fixed top-20 right-6 z-[60] bg-green-600 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center space-x-3 border border-green-500 animate-slide-in">
      ...
      <section className="relative py-20 px-6 text-center bg-gradient-to-b from-green-500/10 via-transparent to-transparent">
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight leading-tight">
            FUTO CSC <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-teal-400">Yellow Pages</span>
          </h1>
          ...
          {/* Search Controls */}
          <div className="flex flex-col sm:flex-row items-center gap-2 max-w-xl mx-auto p-1.5 rounded-xl shadow-lg border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            ...
            <button 
              ...
              className="flex items-center justify-center gap-1.5 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-bold text-xs tracking-wide uppercase transition-all duration-300 rounded-lg shadow-md hover:shadow-green-500/30 cursor-pointer whitespace-nowrap w-full sm:w-auto"
            >
              <FiPlus className="text-sm" /> Add Business
            </button>
          </div>
        </div>
      </section>
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-6 mb-10 scrollbar-hide">
          {categories.map((cat) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-200 whitespace-nowrap cursor-pointer border ${
        ...
        {filteredBusinesses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredBusinesses.map((biz) => (
              <div 
                key={biz.id}
                className={`group flex flex-col rounded-3xl overflow-hidden border shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1.5 ${
                  ...
                  <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-700/50">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <a 
                        ...
                        className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-green-500 hover:bg-green-600 text-white font-bold text-xs uppercase transition-all duration-300 shadow hover:shadow-green-500/20"
                      ...
                      <a 
                        href={`tel:${biz.phone}`} 
                        className={`flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 font-bold text-xs uppercase hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300 ${
          ...
          <div className="text-center py-24 opacity-60 border border-dashed border-gray-300 dark:border-gray-700 rounded-3xl">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className={`relative w-full max-w-2xl rounded-3xl shadow-2xl border overflow-hidden animate-zoom-in ${
            theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-150 text-gray-900'
          }`}>
            {/* Modal Header */}
            ...
            <form onSubmit={handleRegisterBusiness} className="p-6 overflow-y-auto max-h-[70vh] space-y-5 text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 text-gray-900 dark:text-white"
                ...
                <div>
                  ...
                  <select 
                    className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:border-green-500 text-gray-900 dark:text-white"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Owner Name *</label>
                  <input 
                    type="text" 
                    required 
                    placeholder="e.g. John Doe" 
                    className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-transparent text-sm focus:outline-none focus:border-green-500 text-gray-900 dark:text-white"
              ...
              <div className="pt-4 border-t border-gray-200 dark:border-gray-700/50 flex justify-end gap-3">
                <button 
                  type="button" 
                  onClick={() => setIsModalOpen(false)}
                  className="px-6 py-3 border border-gray-300 dark:border-gray-600 text-xs font-bold uppercase rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-gray-900 dark:text-white"
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold text-xs uppercase tracking-wide rounded-xl shadow-lg hover:shadow-green-500/20 transition-all duration-300"
                >
                  Register Business
                </button>
              </div>
